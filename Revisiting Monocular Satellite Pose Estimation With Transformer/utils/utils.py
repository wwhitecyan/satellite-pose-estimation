    ...
    def visualize(self, i, partition='train', ax=None):
        ...
        if partition == 'train':
            ...
            ax.arrow(xa[0], ya[0], xa[1] - xa[0], ya[1] - ya[0],
                     head_width=30, color='r')
            ax.arrow(xa[0], ya[0], xa[2] - xa[0], ya[2] - ya[0],
                     head_width=30, color='g')
            ax.arrow(xa[0], ya[0], xa[3] - xa[0], ya[3] - ya[0],
                     head_width=30, color='b')
...
class PyTorchSatellitePoseEstimationDataset(Dataset):
    ...
    def __init__(self, split='train', speed_root='', transform=None):
        ...
        if split not in {'train', 'test', 'real_test'}:
            raise ValueError('Invalid split, has to be either \'train\','
                             ' \'test\' or \'real_test\'')
        ...
        if self.train:
            self.labels = {label['filename']: {'q': label['q_vbs2tango'],
