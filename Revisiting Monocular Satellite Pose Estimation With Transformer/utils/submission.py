...
class SubmissionWriter:
    ...
    def _append(self, filename, q, r, real):
        if real:
            self.real_test_results.append(
                {'filename': filename, 'q': list(q), 'r': list(r)})
        else:
    ...
    def export(self, out_dir='', suffix=None):
        ...
        sorted_real_test = sorted(
            self.real_test_results, key=lambda k: k['filename'])
        timestamp = datetime.now().strftime("%Y%m%d-%H%M")
        if suffix is None:
            suffix = timestamp
        submission_path = os.path.join(
            out_dir, 'submission_{}.csv'.format(suffix))
        with open(submission_path, 'w') as f:
            csv_writer = csv.writer(f, lineterminator='\n')
            for result in (sorted_test + sorted_real_test):
                csv_writer.writerow(
                    [result['filename'], *(result['q'] + result['r'])])
