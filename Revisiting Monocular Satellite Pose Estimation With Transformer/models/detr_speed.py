...
class DETR(nn.Module):
    ...
    def __init__(self,
                 backbone, transformer, num_queries, aux_loss=False):
        ...
        self.cls_embed = nn.Linear(hidden_dim, 12)
        # self.cls_embed = MLP(hidden_dim, hidden_dim, 12, 3)
    ...
    def forward(self, samples: NestedTensor):
        ...
        hs = self.transformer(
            self.input_proj(src), mask, self.query_embed.weight, pos[-1])[0]

        outputs_class = self.cls_embed(hs)
...
class SetCriterion(nn.Module):
    ...
    def loss_points(self, outputs, targets, indices, num_points):
        """Compute the losses related to the landmarks, the L1 regression loss and the GIoU loss
        ...
        loss_points = smooth_l1_loss(
            src_points, target_points, beta=1./200., reduction='none')
        losses = {}
    ...
    def forward(self, outputs, targets):
        ...
        # Retrieve the matching between the outputs of the last layer and
        # the targets
...
class PostProcess(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, outputs, clip_bbox):
        ...
        assert len(out_logits) == len(clip_bbox)

        prob = F.softmax(out_logits, -1)

        # and from relative [0, 1] to absolute [0, height] coordinates
        for pt, bbox in zip(out_points, clip_bbox):
            width, height = bbox[2] - bbox[0], bbox[3] - bbox[1]
            x1, y1 = bbox[0], bbox[1]
            pt[:, 0] = pt[:, 0] * width + x1
            pt[:, 1] = pt[:, 1] * height + y1

        results = [{'logits': np.asarray(s),
                    'points': np.asarray(p)} for
                   s, p in zip(prob, out_points)]
...
def build(args):
    ...
    weight_dict = {'loss_ce': 1, 'loss_points': args.pts_loss_coef}
    # TODO this is a hack
    if args.aux_loss:
        aux_weight_dict = {}
    ...
    criterion.to(device)
    postprocessors = {'points': PostProcess()}
