...
class SpeedSubmission(Dataset):
    def __init__(self, ann_file, img_dir, resize=512):
        ...
        self.normalize = Normalize(
            [0.485, 0.456, 0.406], [0.229, 0.224, 0.225], resize)
    ...
    def __getitem__(self, idx):
        ...
        cropImg[crop_y1:crop_y1+y2-y1,
                crop_x1:crop_x1+x2-x1] = img[y1:y2, x1:x2]
...
class SpeedTrain(Dataset):
    def __init__(self, ann_file, index_file, img_dir,
                 resize=512, train=True, transforms=None):
        super(SpeedTrain, self).__init__()
        self.ann_file, self.img_dir = ann_file, img_dir
        self.index_file, self.transforms = index_file, transforms
    ...
    def load_anns(self):
        ...
        indexs = np.loadtxt(
            osp.join(DATA_ROOT, 'annos', self.index_file), dtype=int)
    ...
    def __getitem__(self, idx):
        ...
        if self.transforms:
            transformed = self.transforms(
                image=cropImg, keypoints=landmarks)
            cropImg, landmarks = transformed['image'], transformed['keypoints']

        # cropImg = torch.from_numpy(
        #     cropImg).to(torch.float).permute(2, 0, 1) / 255
        cropImg = img_trunc(cropImg, p=.2)
        cropImg = F.to_tensor(cropImg)
    ...
    if train:
        return A.Compose(
            [
                ...
                A.JpegCompression(20, 99, p=.1),
                A.ISONoise(p=.2),
                A.Cutout(num_holes=10, max_h_size=40, max_w_size=40, p=.1),
            ],
...
class SpeedEval(object):
    def __init__(self, gt_file, solver):
        self.solver = solver

        with open(osp.join(DATA_ROOT, gt_file), 'r') as f:
    ...
    def update(self, predictions):
        for filename, ret in predictions.items():
            ...
            score_tvec, score_quat = speed_score(
                quat_pr, tvec_pr, quat_gt, tvec_gt)
            self.log[filename] = {
    ...
    def summarize(self):
        scores = np.asarray(
            [item['score'] for filename, item in self.log.items()])
        ...
        tvec_abs = np.stack([
            np.abs(
                np.asarray(item['tvec_pr']) - np.asarray(item['tvec_gt'])
            )
