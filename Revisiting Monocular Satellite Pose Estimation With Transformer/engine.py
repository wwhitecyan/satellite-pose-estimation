...
from datasets.speed import SpeedEval
...
def train_one_epoch(model: torch.nn.Module,
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        losses = sum(
            loss_dict[k] * weight_dict[k] for k in loss_dict.keys() if
            k in weight_dict)
...
def evaluate(model, criterion, postprocessors, data_loader,
             gt_file, solver, device, output_dir):
    model.eval()
    criterion.eval()
    ...
    speed_evaluator = SpeedEval(gt_file, solver)
    ...
    if speed_evaluator is not None:
        if 'points' in postprocessors.keys():
